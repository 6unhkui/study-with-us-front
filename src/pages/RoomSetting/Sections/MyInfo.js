...
const MyInfo = ({roomId}) => {
    ...
    const { roomDetail : { name, isManager }} = useSelector(state => state.room);
    const { me : { joinDate, postCount, role }} = useSelector(state => state.member);
    ...
    return (
        <Descriptions title="내 정보" bordered>
            <Descriptions.Item label="가입일" span={3}>{joinDate}</Descriptions.Item>
