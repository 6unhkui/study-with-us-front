...
import RegisterAttendance from "containers/RegisterAttendance";
    ...
    return (
        <ContentWrap>
            ...
            <MembersWrap>
                ...
                >
                    {membersAttendance && membersAttendance.filter(v => !v.attendance).map((item, i) => (
                        <Tooltip title={item.name} placement="top" key={i}>
                            <AntAvatar
                                src={item.profileImg}
                                style={{backgroundColor: !item.profileImg && stringToColor(item.name)}}
                            >
