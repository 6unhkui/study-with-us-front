...
import {
    ...
    LOAD_MEMBER_DETAIL_REQUEST,
    LOAD_MEMBER_DETAIL_SUCCESS,
    LOAD_MEMBER_DETAIL_FAILURE,
    DELETE_MEMBER_REQUEST,
    DELETE_MEMBER_SUCCESS,
    DELETE_MEMBER_FAILURE,
    LOAD_MY_DETAIL_REQUEST,
    LOAD_MY_DETAIL_SUCCESS,
    LOAD_MY_DETAIL_FAILURE,
    WITHDRAWAL_REQUEST,
    WITHDRAWAL_SUCCESS,
    WITHDRAWAL_FAILURE,
    CHANGE_MANAGER_REQUEST,
...
function* loadMembers(action) {
    try {
        ...
        } = yield call(loadMembersAPI, action.roomId, action.pagination, action.keyword);
        yield put({
...
function* loadMemberDetail(action) {
    try {
        const {
            data: { data }
        } = yield call(loadMemberDetailAPI, action.memberId);
        yield put({
...
function* loadMyDetail(action) {
    try {
        const {
            data: { data }
        } = yield call(loadMyDetailAPI, action.data);
        yield put({
